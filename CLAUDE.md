...
- Commit changes with a descriptive message in conventional commits format.
- Keep `analysis/explore.ipynb` current — it is the consolidated analysis document (ROADMAP.md and the standalone analysis .md files were merged into it). Edit `analysis/_build_explore_nb.py` (the text source of truth), regenerate with `python analysis/_build_explore_nb.py`, and update §12 "Further work" as items open or close.
